...
```{r}
sapply(df,function(x,na.rm=T) {sum(is.na(x))})
```
...
```{r}
sapply(df,function(x,na.rm=T){max(x)-min(x)})
```
```{r}
checkDIFF<-function(v1)
{
  v2<-ifelse(is.na(v1),mean(v1,na.rm=T),v1)
  v3<-max(v2)-min(v2)
}
...
```
```{r}
v2<-ifelse(is.na(df$A2),mean(df$A2,na.rm=T),df$A2)
v2
```
# Normalization
(x-min())/(max()-min())
...
df<-read.csv("C:/Users/user/Documents/CU data mining & ML/Module 1/Data managment with R-20201228/TrainingData_latest.csv",
...
```
```{r}
attr(sort(sapply(df,function(x){sum(is.na(x))}),decreasing = T)[1:4],"names")
```
...
sub_na<-function(x)
{
  v=ifelse(is.na(x),median(x,na.rm=T),x)
  v
}
